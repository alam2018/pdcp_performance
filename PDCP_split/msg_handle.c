...
//	activeRequests[nSockFd].usage = false;
	free (activeRequests[nSockFd].sockBufferDatabase[0].pData);
	return 0;
}
...
//	isNewCon = true;
	...
	for (i=0; i<MAX_NO_CONN_TO_PDCP; i++)
	{
		...
		{
			activeRequests[i].sockBufferDatabase[0].pData = (UINT8*)malloc(BUFFER_SIZE);
					if (NULL == activeRequests[i].sockBufferDatabase[0].pData)
					{
...

{
	...
	connIndex = schedID;
	temppdcpReceiveBuffer = activeRequests[schedID].sockBufferDatabase[0].pData;
	memset(activeRequests[schedID].sockBufferDatabase[0].pData,0,BUFFER_SIZE);
	...
	switch ( messageId )
	{
		...
		case PDCP_DATA_REQ_FUNC:
		{
			...
			*/
			...
			if (pdcpDataReqFuncMsg.ctxt_pP.enb_flag == 0)
			{
			...
			rohc_decompmain(decompressor[connIndex]);
			...
			} else
			{
				...
				memcpy (sdu_buffer, &pdcpDataReqFuncMsg.rohc_packet.dataBuffer, pdcpDataReqFuncMsg.sdu_buffer_size);
			int tempPacketLength = pdcpDataReqFuncMsg.rohc_packet.len;
			rohc_main(compressor[connIndex]);
...
/*			result = pdcp_data_req (&pdcpDataIndMsg.ctxt_pP, pdcpDataIndMsg.srb_flagP,
					pdcpDataIndMsg.rb_id, pdcpDataIndMsg.muiP, pdcpDataIndMsg.confirmP,
					pdcpDataIndMsg.sdu_buffer_size, sdu_buffer, pdcpDataIndMsg.mode);*/

			result = pdcp_data_ind_uplink (connIndex, &pdcpDataIndMsg.ctxt_pP, pdcpDataIndMsg.srb_flagP,
					pdcpDataIndMsg.MBMS_flagP, pdcpDataIndMsg.rb_id, pdcpDataIndMsg.sdu_buffer_size, sdu_buffer);
#else
			boolean_t result = pdcp_data_ind_uplink (connIndex, &pdcpDataIndMsg.ctxt_pP, pdcpDataIndMsg.srb_flagP,
