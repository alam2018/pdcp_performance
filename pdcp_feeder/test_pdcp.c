...
#define NUMBER_OF_TEST_PACKETS 1500
/*
...
//Define the data direction. If PDCP feeder acts as SDAP then the data direction is downlink and vice versa
bool pdcpFeederAsSDAP = true;
...
extern PDCP_DATA_REQ_FUNC_T				pdcpDataReqMsg;
...
//    if (pdcp_data_req(0, 0, 10, DUMMY_BUFFER, &pdcp_array[0], &test_pdu_tx_list) == TRUE) {
    bool ret_res = msg_send (2);

if (isUplink == 1)
{
    pdcpFeederAsSDAP = false;
    ret_res = msg_send (3);
    pdcpFeederAsSDAP = true;
}
...
//      mem_block_t* pdcp_test_pdu = list_remove_head(&test_pdu_tx_list);
      ...
      mem_block_t pdcp_test_pdu;
      pdcp_test_pdu.data = &pdcpDataReqMsg.pdcp_pdu.data;

      // Alam, Verify the cyphering
#if defined(ENABLE_SECURITY)
      unsigned char* tempCharPtr = 0;
      int tempInt = 0x05;
      tempCharPtr = (unsigned char*)(&tempInt);

      pdcp_array[0].security_activated = *tempCharPtr - 0x01;
      pdcp_array[0].cipheringAlgorithm = *tempCharPtr - 0x03;
...
//      pdcp_array[0].kUPenc = &key;

      uint8_t                            *kUPenc = NULL;
      algorithm_type_dist_t alg_type = 0x05;
      uint8_t alg_id = (uint8_t) pdcp_array[0].cipheringAlgorithm;
      derive_key_up_enc(alg_id, key, &kUPenc);
      pdcp_array[0].kUPenc = kUPenc;


      pdcp_t *pdcp_p = &pdcp_array[0];
      protocol_ctxt_t* ctxt_pP = &pdcpDataReqMsg.ctxt_pP;
      uint8_t        pdcp_header_len = PDCP_USER_PLANE_DATA_PDU_LONG_SN_HEADER_SIZE;
      uint16_t       current_sn = (uint16_t) index;
      uint16_t SDU_size = (uint16_t)pdcpDataReqMsg.sdu_buffer_size;
      pdcp_validate_security (ctxt_pP, pdcp_p, pdcpDataReqMsg.srb_flagP, pdcpDataReqMsg.rb_id,
    		  pdcp_header_len, current_sn, pdcp_test_pdu.data, SDU_size);

//      uint8_t *buffer_decrypted = NULL;
//      buffer_decrypted = pdcp_test_pdu->data + pdcp_header_len;
#endif


 //     memcpy(pdcp_test_pdu->data, &pdcpDataReqMsg.pdcp_pdu, sizeof(pdcpDataReqMsg.pdcp_pdu));
...
/*       * Verify that this is a data packet by checking
      ...
      int val = pdcp_test_pdu_buffer[0];
      if (pdcp_test_pdu_buffer[0] & 0x80) {
//        msg("[TEST] First bit is not 0, which means this is not a Data PDU!\n");
    	  msg("[TEST] First bit is not 0, which means this is a Data PDU!\n");
//        return FALSE;
      } else {
        msg("[TEST] First bit is 1, so this is not a Data PDU, BAD\n");
        return FALSE;
      }
...
//      if ((pdcp_test_pdu_buffer[0] & 0xF0) != 0) {
...
//      u16 sequence_number = msg_send (4);
